...
test.describe('B1 Pages landing', () => {
  test('Home responds HTTP 200', async ({ page }) => {
    const response = await page.goto(B1_PAGES_URL, { waitUntil: 'domcontentloaded' });
    expect(response?.status(), `${B1_PAGES_URL} should return 200`).toBe(200);
  });

  test('Home is a code leaf: no unpublished-result tokens', async ({ page }) => {
    await page.goto(B1_PAGES_URL, { waitUntil: 'domcontentloaded' });
  ...
  test('Sticky Homepage + SCPortal chrome', async ({ page }) => {
    await page.goto(B1_PAGES_URL, { waitUntil: 'networkidle' });
  ...
  test('No journal-submission packaging', async ({ page }) => {
    ...
    const html = await page.content();
    const text = await visibleBodyText(page);
    expect(findSubmissionPackagingLeaks(text), `submission packaging leaks on ${B1_PAGES_URL}`).toEqual([]);
    expect(hasBibTeXKit(html), 'BibTeX kit present on unpublished leaf').toBe(false);
  });

  test('Public repo href is the b1-site GitHub repo', async ({ page }) => {
    await page.goto(B1_PAGES_URL, { waitUntil: 'networkidle' });
    const codeLink = page.locator(`a[href="${B1_GITHUB_URL}"]`).first();
    await expect(codeLink, 'Link to PeterPonyu/b1-site missing').toBeVisible();
  });
  ...
  for (const url of STUB_ROUTES) {
    test(`Stub route stays 200 without restored results: ${new URL(url).pathname}`, async ({ page }) => {
      const response = await page.goto(url, { waitUntil: 'domcontentloaded' });
      expect(response?.status(), `${url} should stay the 200 stub, not a restored results page`).toBe(200);
      const html = await page.content();
      const text = await visibleBodyText(page);
      expect(findResultLeaks(`${text}\n${html}`), `unpublished result tokens on ${url}`).toEqual([]);
    });
  }

  for (const url of UNPUBLISHED_ASSETS) {
    test(`Unpublished figure stays 404: ${new URL(url).pathname}`, async ({ page }) => {
      const response = await page.goto(url, { waitUntil: 'domcontentloaded' });
      expect(response?.status(), `${url} must stay 404, not a restored figure`).toBe(404);
    });
  }
});
